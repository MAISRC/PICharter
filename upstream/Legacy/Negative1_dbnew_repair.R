...
compiled.folder = googledrive::drive_get("https://drive.google.com/drive/folders/1DgBorlz_n0RLPKCpQsA0OAlOgXt8UG_A")$id #Grab.
needloc.folder = googledrive::drive_get("https://drive.google.com/drive/folders/1DpG5FCZka1sXx-ztuEOitVA8MpRdx5r4")$id #Grab.
locreq.folder = googledrive::drive_get("https://drive.google.com/drive/folders/1y23V14XXWNY0sb-0ri6-Sx250uKYxT9H")$id #Grab.

#Get a list of the files (as IDs) in the approved folder. We will load and process them one at a time.
content.ids = c(drive_ls(needloc.folder)$id, drive_ls(compiled.folder)$id, drive_ls(locreq.folder)$id) #The contents of the folder as Gdrive IDs.
...
blah = rbind(drive_ls(needloc.folder), drive_ls(compiled.folder), drive_ls(locreq.folder))
...
for(n in 1:length(content.ids)) {
  ...
  if(any(c("Dulichium.arundinaceum","Eleocharis", "potamogeton_zostriformis", "ultricularia_intermedia", "dulichium.arundinaceum", "chara_sp.", "drepanocladus_fontinalis_sp", "lemna_sp.", "lychnothamus_barbatus", "valisneria_americana"
  ) %in% names(current.import))) { print(n); print("Bad name!") 
    print(c("Dulichium.arundinaceum","Eleocharis", "potamogeton_zostriformis", "ultricularia_intermedia", "dulichium.arundinaceum", "chara_sp.", "drepanocladus_fontinalis_sp", "lemna_sp.", "lychnothamus_barbatus", "valisneria_americana"
    )[which(c("Dulichium.arundinaceum","Eleocharis", "potamogeton_zostriformis", "ultricularia_intermedia", "dulichium.arundinaceum", "chara_sp.", "drepanocladus_fontinalis_sp", "lemna_sp.", "lychnothamus_barbatus", "valisneria_americana"
    ) %in% names(current.import))])}
  
  #Fix a little screw-up of mine...
  if(any(grepl("vallisnaria", names(current.import)))) {
    names(current.import) = gsub("vallisnaria", "vallisneria", names(current.import))
    print("Yup")
    fn = blah$name[blah$id == id]
    write.csv(x = current.import, file = paste0(fn))
  }
